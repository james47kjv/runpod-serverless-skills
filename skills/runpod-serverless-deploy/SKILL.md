...
Every deploy using this skill MUST satisfy ALL of these. No exceptions.
 ...
 3. **Model via** `modelName` **+** `flashboot: true`**.** Weights NOT baked into image.
...
14. **HF cache MUST read from** `/runpod-volume/huggingface-cache/hub`**when present** (RunPod's host-disk cache, populated by `spec.modelName`). Two-tier pattern: read on `/runpod-volume`, write on `/tmp`. Anchoring at `/tmp` defeats FlashBoot — every scale-from-zero re-downloads the model (3-5 min for 7-8 GB models). `/runpod-volume` here is NOT a user-attached Network Volume; it's host-local and does NOT pin region. Spec env MUST NOT hard-code `HF_HOME` or `HUGGINGFACE_HUB_CACHE` — let runtime detection in [start.sh](http://start.sh) / [boot.py](http://boot.py) decide. Diagnostic: emit `hf_cache_resolved` event showing whether `location` is `runpod-host` or `tmp-fallback`. See pitfall #30 / setup-guide section 6.1.7.
...
18. **RunPod Model field verification is empirical, not REST-readback.** REST GET no longer exposes `modelName`, so `body.get("modelName") is None` is not proof the UI Model field is empty. The durable spec and UI Model field must use the bare private repo id only, e.g. `james47kjv/nonnon-vl-embedder`; do not paste `https://huggingface.co/...:<sha>` or any `:<revision>` suffix into the UI Model field. The pinned revision belongs in template env (`NONNON_HF_MODEL_REVISION`, `LAMP_HF_MODEL_REVISION`, or equivalent). After any Model-field UI save, run `python C:/Users/innas/architecture/scripts/monitoring/runpod_drift_monitor.py --autoheal`, then verify the next cold boot from container logs: `hf_cache_resolved location=runpod-host` plus `model_cache_hit`. If logs show `tmp-fallback`, `model_cache_miss`, or a live `snapshot_download` path with `NONNON_ALLOW_HF_SELF_HEAL=0`, fix the Model field/cache path instead of raising `workersMin`.
...
**Gotcha:** do NOT include `modelName` in an update-only `saveEndpoint`mutation. Set it once at create time via `deploy_endpoint.py`; including it mid-rollout causes 500 (pitfall 18).
